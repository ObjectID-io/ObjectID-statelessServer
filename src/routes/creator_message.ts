...
export default async function creator_message(req: Request, res: Response) {
  try {
    ...
    const moveFunction = packageID + "::oid_object::creator_message";
    ...
    singAndExecTx(network, client, gasStation, useGasStation, keyPair, tx, {
      onSuccess: (result: IotaTransactionBlockResponse) => {
        ...
        if (status.status === "success") {
          ...
          console.error("creator_message failed.");
        }
      },
      onError: (err: any) => {
        res.json({ success: false, error: err });
        console.error("creator_message failed:", err);
      },
      onSettled: () => {},
    });
  } catch (err) {

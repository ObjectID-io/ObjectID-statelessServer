...
export default async function add_approver_did(req: Request, res: Response) {
  try {
    const { seed, network, controllerCap, document, new_approver_did } = req.body;

    logInputs("add_approver_did", {
      network,
      controllerCap,
      document,
      new_approver_did,
    ...
    const moveFunction = documentPackageID + "::oid_document::add_approver_did";

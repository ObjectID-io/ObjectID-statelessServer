...
export default async function create_component(req: Request, res: Response) {
  try {
    const { seed, network, creditToken, controllerCap, object, component_id, description } = req.body;

    logInputs("create_component", { network, creditToken, controllerCap, object, component_id, description });

    const { client, keyPair, policy, gasStation, packageID } = await setupEnv(seed, network);

    const tx = new Transaction();
    const moveFunction = packageID + "::oid_object::create_component";
    ...
    singAndExecTx(network, client, gasStation, useGasStation, keyPair, tx, {
      onSuccess: (result: IotaTransactionBlockResponse) => {
        ...
        if (status.status === "success") {
          ...
          console.error("create_component failed.");
        }
      },
      onError: (err: any) => {
        res.json({ success: false, error: err });
        console.error("create_component failed:", err);
      },
      onSettled: () => {},
    });
  } catch (err) {

...
export default async function delete_component(req: Request, res: Response) {
  try {
    const { seed, network, creditToken, controllerCap, object, component } = req.body;

    logInputs("delete_component", { network, creditToken, controllerCap, object, component });

    const { client, keyPair, policy, gasStation, packageID } = await setupEnv(seed, network);

    const tx = new Transaction();
    const moveFunction = packageID + "::oid_object::delete_component";
    ...
    singAndExecTx(network, client, gasStation, useGasStation, keyPair, tx, {
      onSuccess: (result: IotaTransactionBlockResponse) => {
        ...
        if (status.status === "success") {
          ...
          console.error("delete_component failed.");
        }
      },
      onError: (err: any) => {
        res.json({ success: false, error: err });
        console.error("delete_component failed:", err);
      },
      onSettled: () => {},
    });
  } catch (err) {

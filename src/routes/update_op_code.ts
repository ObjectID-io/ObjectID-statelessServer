...
export default async function update_op_code(req: Request, res: Response) {
  try {
    const { seed, network, creditToken, controllerCap, object, new_op_code } = req.body;

    logInputs("update_op_code", { network, creditToken, controllerCap, object, new_op_code });

    const { client, keyPair, policy, gasStation, packageID } = await setupEnv(seed, network);

    const tx = new Transaction();
    const moveFunction = packageID + "::oid_object::update_op_code";
    ...
    singAndExecTx(network, client, gasStation, useGasStation, keyPair, tx, {
      onSuccess: (result: IotaTransactionBlockResponse) => {
        ...
        if (status.status === "success") {
          ...
          console.error("update_op_code failed.");
        }
      },
      onError: (err: any) => {
        res.json({ success: false, error: err });
        console.error("update_op_code failed:", err);
      },
      onSettled: () => {},
    });
  } catch (err) {

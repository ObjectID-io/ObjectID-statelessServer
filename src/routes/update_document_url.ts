...
export default async function update_document_url(req: Request, res: Response) {
  try {
    const { seed, network, controllerCap, document, new_document_url } = req.body;

    logInputs("update_document_url", {
      network,
      controllerCap,
      document,
      new_document_url,
    ...
    const moveFunction = documentPackageID + "::oid_document::update_document_url";

...
export default async function update_document_owner_did(req: Request, res: Response) {
  try {
    const { seed, network, controllerCap, document, new_owner_did } = req.body;

    logInputs("update_document_owner_did", {
      network,
      controllerCap,
      document,
      new_owner_did,
    ...
    const moveFunction = documentPackageID + "::oid_document::update_owner_did";

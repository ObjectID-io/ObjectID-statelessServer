...
export default async function message(req: Request, res: Response) {
  try {
    const { seed, network, creditToken, controllerCap, object, message_code, message, geolocation, link } = req.body;

    logInputs("message", { network, creditToken, controllerCap, object, message_code, message, geolocation, link });

    const { client, keyPair, policy, gasStation, packageID } = await setupEnv(seed, network);

    const tx = new Transaction();
    const moveFunction = packageID + "::oid_object::message";

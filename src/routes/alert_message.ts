...
export default async function alert_message(req: Request, res: Response) {
  try {
    ...
    const moveFunction = packageID + "::oid_object::alert_message";
    ...
    singAndExecTx(network, client, gasStation, useGasStation, keyPair, tx, {
      onSuccess: (result: IotaTransactionBlockResponse) => {
        ...
        if (status.status === "success") {
          ...
          console.error("alert_message failed.");
        }
      },
      onError: (err: any) => {
        res.json({ success: false, error: err });
        console.error("alert_message failed:", err);
      },
      onSettled: () => {},
    });
  } catch (err) {

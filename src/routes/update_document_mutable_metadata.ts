...
export default async function update_document_mutable_metadata(req: Request, res: Response) {
  try {
    const { seed, network, controllerCap, document, new_mutable_metadata } = req.body;

    logInputs("update_document_mutable_metadata", {
      network,
      controllerCap,
      document,
    ...
    const moveFunction = documentPackageID + "::oid_document::update_document_mutable_metadata";

...
export default async function update_object_did(req: Request, res: Response) {
  try {
    const { seed, network, creditToken, controllerCap, object, new_object_did } = req.body;

    logInputs("update_object_did", { network, creditToken, controllerCap, object, new_object_did });

    const { client, keyPair, policy, gasStation, packageID } = await setupEnv(seed, network);

    const tx = new Transaction();
    const moveFunction = packageID + "::oid_object::update_object_did";
    ...
    singAndExecTx(network, client, gasStation, useGasStation, keyPair, tx, {
      onSuccess: (result: IotaTransactionBlockResponse) => {
        ...
        if (status.status === "success") {
          ...
          console.error("update_object_did failed.");
        }
      },
      onError: (err: any) => {
        res.json({ success: false, error: err });
        console.error("update_object_did failed:", err);
      },
      onSettled: () => {},
    });
  } catch (err) {

...
export default async function update_document_status(req: Request, res: Response) {
  try {
    const { seed, network, controllerCap, document, new_status } = req.body;

    logInputs("update_document_status", {
      network,
      controllerCap,
      document,
      new_status,
    ...
    const moveFunction = documentPackageID + "::oid_document::update_document_status";

...
export function proposeControllerExecution(identity, capability, controllerCapId, packageId, expiration) {
...
export function executeControllerExecution(identity, capability, proposalId, controllerCapRef, intentFn, packageId) {
    const tx = new Transaction();
    const cap = tx.objectRef(capability);
    const [delegationToken, borrow] = getControllerDelegation(tx, cap, packageId);
...
export function createAndExecuteControllerExecution(identity, capability, controllerCapRef, intentFn, packageId, expiration) {
    const tx = new Transaction();
    const cap = tx.objectRef(capability);
    const [delegationToken, borrow] = getControllerDelegation(tx, cap, packageId);
    const identityArg = tx.sharedObjectRef(identity);

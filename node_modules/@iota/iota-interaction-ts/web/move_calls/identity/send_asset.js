...
export function proposeSend(identity, capability, transferMap, packageId, expiration) {
...
export function executeSend(identity, capability, proposalId, objects, packageId) {
    const tx = new Transaction();
    const cap = tx.objectRef(capability);
    const [delegationToken, borrow] = getControllerDelegation(tx, cap, packageId);
